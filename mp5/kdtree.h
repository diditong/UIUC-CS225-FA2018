...
class KDTree
{
  ...
  public:
    ...
    void fnn(KDTreeNode * node, Point<Dim> & query, Point<Dim> & curBest, int curDim) const;
  ...
  private:
    ...
    /**
     * Helper function for constructing a KDTree.
     *
     * @param newPoints Vector of new poitns that needs to populate the tree.
     * @param left Lower bound from the vector (inclusive).
     * @param right Upper bound from the vector (inclusive).
     * @param curDim Current dimention to do quick select.
     * @param subroot The root that a node should be manipulated.
     */
    void constructKDTree(vector<Point<Dim>>& newPoints, int left, int right,
                         int curDim, KDTreeNode *& subroot);

    /**
     * QuickSelect algorithm that accepts the current dimention and returns
     * the kth element from the vector of points.
     *
     * @param points Vector of new poitns that needs to populate the tree.
     * @param left Lower bound from the vector (inclusive).
     * @param right Upper bound from the vector (inclusive).
     * @param k The position of the element we want.
     * @param curDim Current dimention to do quick select.
     * @return kth point from the vector.
     */
    Point<Dim> quickSelectMedian(vector<Point<Dim>> & points, int left,
                                 int right, size_t k, int curDim);

    /**
     * Partition algorithm for quick select.
     *
     * @param points Vector of new poitns that needs to populate the tree.
     * @param left Lower bound from the vector (inclusive).
     * @param right Upper bound from the vector (inclusive).
     * @param pivot pivot of the vector.
     * @param curDim Current dimention to do quick select.
     * @return the new pivot index of the vector.
     */
    size_t partition(vector<Point<Dim>> & points, int left, int right,
                     size_t pivot, int curDim);

    /**
     * Helper function for copy contructor and assignment operator.
     *
     * @param node The target object to copy.
     * @param otherNode The object to be copied.
     */
    void _copy(KDTreeNode *& node, KDTreeNode *& otherNode);

    /**
     * Helper function for destructor.
     *
     * @param node The target object to delete.
     */
