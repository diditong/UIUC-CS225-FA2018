...
int binarySearch(const std::vector<T>& vector, int first, int last, const C key )
{
  if (last < first)
    return first;
  int mid = (first + last)/2;
  if (vector.at(mid) == key)
    return mid;
  else if (vector.at(mid)< key)
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return binarySearch(elements,0,size-1, val);
}
