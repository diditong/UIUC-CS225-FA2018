...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    if (dict.find(w1) == dict.end() || dict.find(w2)== dict.end() || (dict.find(w1)->second.size() != dict.find(w2)->second.size()))
      return false;

    std::vector<std::string> p1 = dict.find(w1)->second;
    std::vector<std::string> p2 = dict.find(w2)->second;
